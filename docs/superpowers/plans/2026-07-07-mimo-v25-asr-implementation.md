...
- MiMo segment submission is upgraded from serial execution to bounded parallel execution: request launches are globally rate-limited, at most five requests can be active at the same time, and results are still merged in original segment order. Real E2E showed one launch per second can trigger MiMo 429/TPM limits on 180s segments, so the default launch interval is tuned to eight seconds while still satisfying the user's "at most one request in a one-second window" constraint. Segment-level HTTP 429 retries use 2s, 4s, 8s, and 16s backoff.
