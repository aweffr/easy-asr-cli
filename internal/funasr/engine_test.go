...
func TestEngineEmitsProgressEvents(t *testing.T) {
	...
	runner := funasr.NewEngine(funasr.Options{
		Config:  config.Default().FunASR(),
		Storage: &fakeStorage{stored: storage.StoredObject{Bucket: "bucket", Key: "tmp/sample.wav"}, url: "https://signed.example/sample.wav"},
		ASR: &fakeASR{
			taskID: "task-fun",
			result: dashscope.FunASRTaskResult{
				TaskID:           "task-fun",
				TranscriptionURL: "https://result.example/fun.json",
				UsageSeconds:     8,
			},
			transcription: srt.Transcription{Transcripts: []srt.Transcript{{
